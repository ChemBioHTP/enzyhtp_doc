...
``parameterizer_temp_dir``
    ...
    (String, optional, default ``{SCRATCH_DIR}/amber_parameterizer``)

    .. admonition:: About ``SCRATCH_DIR``

        ``SCRATCH_DIR`` is a directory for scratch use, which can be defined by yourselves.
